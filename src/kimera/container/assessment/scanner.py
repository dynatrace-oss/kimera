...
class SecurityScanner:
    ...
    def __init__(self, k8s_client: K8sClient, logger: SecurityLogger):
        """Initialize security scanner with Kubernetes client and logger."""
        self.k8s = k8s_client
        self.logger = logger
    ...
    def assess_network_policies(self) -> list[str]:
        ...
        console.print(f"\n[green]--- Network Policies ({self.k8s.namespace}) ---[/green]")

        try:
            policies = self.k8s.list_network_policies(self.k8s.namespace)
        except Exception as e:
            console.print(f"  Error listing network policies: {e}")
            return ["Error checking policies"]

        issues: list[str] = []
        if not policies:
            console.print(f"  ❌ No NetworkPolicies in namespace {self.k8s.namespace}")
            console.print("  ⚠️  All pods can communicate freely (flat network)")
            issues.append("No NetworkPolicies")
        else:
            console.print(f"  Found {len(policies)} NetworkPolicies:")
            has_default_deny = False
            for policy in policies:
                name = policy.metadata.name
                policy_types = policy.spec.policy_types or []
                console.print(f"    • {name} (types: {', '.join(policy_types)})")

                pod_sel = policy.spec.pod_selector
                is_empty_selector = not pod_sel or not pod_sel.match_labels
                if is_empty_selector and "Ingress" in policy_types and "Egress" in policy_types:
                    has_default_deny = True

            if not has_default_deny:
                console.print("  ⚠️  No default-deny policy found")
                issues.append("Missing default-deny policy")
            else:
                console.print("  ✅ Default-deny policy in place")
    ...
    def quick_security_check(self, services: list[str]) -> None:
        ...
        for service, issues, secure in results:
            status = "✅ " + ", ".join(issues) if secure else "⚠️  " + ", ".join(issues)
    ...
    def _assess_runtime_state(self, pod_name: str) -> None:
        ...
        try:
            # Get pod details to identify the container name
            ...
            if not container_name:
                console.print("  ⚠️  No container found in the pod")
                return

            user_info = self.k8s.exec_in_pod(pod_name, "id", container=container_name).strip()
            ...
            if caps:
                ...
                if "CapEff:" in caps:
                    ...
                    if cap_value != "0000000000000000":
                        console.print("    ⚠️  Non-zero effective capabilities detected")

            if pod.spec.host_pid or pod.spec.host_network:
                console.print("  ⚠️  Host namespace access:")
    ...
    def _print_security_context(
        self, sec_context: V1PodSecurityContext | V1SecurityContext, indent: int = 0
    ) -> None:
    ...
    def _print_resources(self, resources: Any, indent: int = 0) -> None:
        """Display resource configuration."""
        prefix = " " * indent

        if not resources:
            console.print(f"{prefix}❌ No resource configuration available")
            return
